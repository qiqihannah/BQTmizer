...
def bqtmizer(dataset_path: str, effectiveness_list: list, cost_list: list, token: str, weights: dict = None, N:int = 30, beta:float = 0.9):
    ...
    try:
        ...
        sampler = Sampler(len(df), beta, N)
        M = sampler.random_sample_unique()
        print(str(M)+" sub-problems are generated.")
        sample_total_list = sampler.bootstrap_sampling(M)
        file_name = os.path.splitext(os.path.basename(validated_path))[0]
        # Quantum annealing results
        qa_solver = QA_solver(N, df, effectiveness_list, cost_list, weights)
        sample_first_list, qpu_access_list, qubit_avg, log_df, sampleset_list = qa_solver.run_qpu(
            sample_total_list, N, token)

        result_builder = ResultBuilder(sample_first_list, df, cost_list, effectiveness_list, weights)
        merge_sample = result_builder.merge()
        result_df, sum_df = result_builder.print_results(merge_sample, qpu_access_list, qubit_avg, cost_list,
                                                         effectiveness_list, weights)
