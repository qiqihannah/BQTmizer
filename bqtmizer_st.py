...
import zipfile
import io
from bootqa import Sampler, QA_solver, ResultBuilder

class BQTmizer_workflow():
    def __init__(self):
        super().__init__()
    # A function to zip a list of DataFrames into a single ZIP file in memory
    def create_zip(self, dataframes):
    ...
    def BQTmizer(self):
                ...
                if None in weights.values():
                    ...
                    if abs(total_weight - 1.0) > 0.01:  # Allowing some floating point tolerance
                        ...
                        if st.button("Run Quantum Annealing"):
                            ...
                            sampler = Sampler(len(df), coverage_rate, subproblem_size)
                            ...
                            qa_solver = QA_solver(subproblem_size, df, effective_features, cost_features, weights)
                            sample_first_list, qpu_access_list, qubit_avg, log_df, sampleset_list = qa_solver.run_qpu(
                                sample_total_list, subproblem_size, api_token)

                            result_builder = ResultBuilder(sample_first_list, df, cost_features, effective_features, weights)
                            merge_sample = result_builder.merge()
                            result_df, sum_df = result_builder.print_results(merge_sample, qpu_access_list, qubit_avg, cost_features,
        ...
        if 'log_df' in st.session_state:
            ...
            zip_file_buffer = self.create_zip(sampleset_with_name)

            # Convert individual DataFrames to CSV for separate downloads
            csv_log = self.convert_df_to_csv(st.session_state['log_df'])
            csv_result = self.convert_df_to_csv(st.session_state['result_df'])
            csv_sum = self.convert_df_to_csv(st.session_state['sum_df'])
